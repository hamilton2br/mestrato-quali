...
Finalmente, para persistir a relação evidência-origem e garantir a sua integridade, \fancyname calcula o hash do par [evidência, identificador da imagem do contêiner] e armazena a tripla [hash do conteúdo da evidência, hash que identifica univocamente o recurso, evidência].
%
...
Um segundo experimento teve como objetivo determinar se, através da análise das evidências coletadas, era possível identificar injeção de malware na memória do contêiner.
%
...
%
Após cinco minutos de \fancyname realizando coletas, uma biblioteca foi injetada na memória de um dos contêiners e permitiu-se que a solução continuasse coletando por mais 5 minutos.
...
%
Embora as coletas via cópia do diretório \textit{numa\_maps} tenham sido bem sucedidas, não foi possível realizar a cópia crua da memória do processo do contêiner via \textit{GDB}. 
%
Segundo Fabio Kung (referencia) isto ocorre porque o \textit{GDB} foi criado antes da existência de \textit{cgroups} no \textit{kernel} do linux e sendo assim não tem consciência da existência de isolamento entre processos.
%
Quando o \textit{GDB} tenta acessar uma área de memória isolada por \textit{cgroups}, o \textit{kernel} envia um sinal de violação de acesso de memória, o resultado é mostrado na Figura \ref{fig:erro-copia-gdb}.
...
%
As alternativas encontradas para cópia da memória de contêiner são específicas de versão e implementação de contêiner como \textit{nsenter} (referencia).
%
Sendo assim o presente trabalho optou por limitar a análise apenas as evidências coletadas via cópia do diretório \textit{numa\_maps}.
...
Finalmente, a terceira fase esperada para o projeto, que consiste no próximo passo para a conclusão do mestrado, tem por objetivo a realização da análise experimental de alguma vulnerabilidade específica, utilizando-se do material coletado na primeira fase do projeto.
%
