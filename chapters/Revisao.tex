...
%
Este capítulo descreve o estado da arte e discute onde a presente proposta contribui para melhorar os trabalhos existentes.

\marcos{Separe em seções essa discussão: isso facilita muito a leitura. Também precisa traduzir as figuras: seu texto está em português, então não faz sentido suas figuras estarem em inglês. Isso vale para qualquer figura, tabela, etc., mesmo se forem de outros trabalhos. Simplesmente diga que foi ``Adaptado de REFERENCIA'' quando for apresentar a figura - Hamilton: Opine por favor}

\section{Estado da Arte}
\label{sec:estadodaarte}

Nesta seção são descritas as propostas existentes hoje para coleta de evidências forenses.
...
\begin{figure}[htb!]
\footnotesize
\caption{\textit{Modelo Automático de Aquisição de Dados Forenses}}
...
\subsection{Arcabouço forense para Open Stack}
\label{sec:frost}
...
Nesta seção, os trabalhos mencionados acima são agrupados e avaliados com base nos diferentes aspectos que abordam.
