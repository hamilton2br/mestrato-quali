...
\marcos{é sempre ``a Figura X'' (maiúsculo, porque é nome próprio), e ``essa figura'' (minúscula, porque não é nome próprio''}
...
A ferramenta FROST (\textit{FoRensic Open Stack Tools} -- Ferramentas Forenses para Arcabouço Open Stack), proposta por \cite{DykstraFROST:2013}, consiste em um conjunto de bibliotecas integradas ao OpenStack \marcos{REFERENCIA}, um dos arcabouços de gerenciamento de infra estruturas virtualizadas bastante difundido.
%
